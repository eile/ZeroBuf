...
def emit():
    ...
    def table():
        ...
        if emit.offset == 4: # OPT: table has no data
            ...
            # move ctor
            emitFunction( None,
                         "{0}( {0}&& rhs )".format( item[1] ),
                        ": ::zerobuf::Zerobuf( std::move( rhs ))\n" +
                        "{\n" + initializer_setters(emit) + "}\n",
                        explicit = False )
